...
def draw_pose_from_cords(pose_joints, img_size, radius=2, draw_joints=True):
    ...
    if draw_joints:
        for f, t in LIMB_SEQ:
            from_missing = MISSING(pose_joints[f][0]) or MISSING(pose_joints[f][1])
            to_missing = MISSING(pose_joints[t][0]) or MISSING(pose_joints[t][1])
            ...
            valid_ids = [i for i in range(len(yy)) if 0 < yy[i] < img_size[0] and 0 < xx[i] < img_size[1]]
...
def load_pose_cords_from_strings(y_str, x_str):
    ## 20181114: FIX bug, convert pandas.Series object to a string-formatted int list
    if isinstance(y_str, Series):
        y_str = y_str.values[0]
    if isinstance(x_str, Series):    
        x_str = x_str.values[0]
    y_cords = json.loads(y_str)
    x_cords = json.loads(x_str)
    # 20191117: modify PATN processed coords by adding 40 to non-negative indices
    # NOTE: For fasion dataset only.
    # print(x_cords)
    # 20191123: deprecate this.
    # x_cords = [item + 40 if item > 0 else item for item in x_cords]
    # print(x_cords)
